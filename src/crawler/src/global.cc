...
const char *global::userAgent;
const char *global::sender;
char *global::headers;
...
global::global (int argc, char *argv[]) {
  const char *configFile = "larbin.conf";
#ifdef RELOAD
  bool reload = true;
#else
  bool reload = false;
  ...
  if (sigaction(SIGPIPE, &sn, &so)) {
    std::cout << "Unable to disable SIGPIPE : " << strerror(errno) << std::endl;
  }
}
  ...
  while (tok != NULL) {
	if (!strcasecmp(tok, "UserAgent")) {
      ...
      if (u->isValid()) {
        ...
        std::cout << "the start url " << tok << " is invalid\n";
        exit(1);
      }
	  ...
	  if ((hp = gethostbyname(tok)) == NULL) {
		endhostent();
		std::cout << "Unable to find proxy ip address (" << tok << ")\n";
		exit(1);
	  } else {
	  ...
	  std::cout << "bad configuration file : " << tok << "\n";
	  exit(1);
	}
	tok = nextToken(&posParse);
  }
...
void global::manageDomain (char **posParse) {
  ...
  if (tok == NULL) {
	std::cout << "Bad configuration file : no end to limitToDomain\n";
	exit(1);
  }
}
...
void global::manageExt (char **posParse) {
  ...
  if (tok == NULL) {
	std::cout << "Bad configuration file : no end to forbiddenExtensions\n";
	exit(1);
  }
}
